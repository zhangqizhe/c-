...
public:
	...
	//后置递增运算符的重载形式，接受一个int参数。通常用于表示在对象递增之前返回对象的副本。
	//在函数体内，对象递增之前的值被保存，并返回该值的副本。递增操作完成后，对象的值会发生变化。
	...
	//前置递增运算符的重载形式，它没有参数。通常用于表示在对象递增之后返回递增后的对象的引用。
	//在函数体内，对象递增之前的值不需要被保存，因为函数返回的是递增后的对象的引用。递增操作完成后，对象的值会发生变化。
	CComplex& operator++()
	{
		mreal += 1;
		mimage += 1;
